<template>
  ...
  <section id="recent-produk">
    <div class="container">
      <h4 class="fw-bold">
        Our <span> Products </span> In Category
        <span>{{ nama_kategori }} </span>
        <i class="icon fa-solid fa-align-center"></i>
      </h4>
      <div class="row">
        <div class="col-md-3" v-for="(item, index) in produk" :key="index">
          <div class="card mb-5">
            <img
...
export default {
  ...
  created() {
    axios
      .get(config.url + `api/produk/kategori/${this.$route.params.id}`)
      .then((res) => {
        (this.produk = res.data.produk),
          (this.nama_kategori = res.data.kategori);
      });
  },
};
</script>